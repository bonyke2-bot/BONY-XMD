"use strict";
/**
 * MakeStore.js - BONY XMD
 * BONY KE TECH - bonyxmd.co.ke
 */
...
const makeStore = () => {
    ...
    const bind = (ev) => {
        ev.on("messaging-history.set", ({ contacts: newContacts, messages: newMessages }) => {
            ...
            if (Array.isArray(newMessages)) {
                for (const m of newMessages) {
                    ...
                    chats[jid].push(m);
                    if (chats[jid].length > MAX_MSGS_PER_CHAT) chats[jid] = chats[jid].slice(-MAX_MSGS_PER_CHAT);
        ...
        ev.on("contacts.upsert", (newContacts) => {
            for (const contact of newContacts) contacts[contact.id] = {...(contacts[contact.id] || {}),...contact };
            trimContacts();
        });
    ...
    const loadMessage = (jid, id) => {
        ...
        if (mapped) {
            ...
            if (Array.isArray(mappedMsgs)) return mappedMsgs.find((m) => m?.key?.id === id) || null;
        }
        return null;
    };
    ...
    return { bind, contacts, chats, messageMap, loadMessage, getMessage, writeToFile };
};
