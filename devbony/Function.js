// BONY XMD - bonyxmd.co.ke - devbony/Function.js
	...
	return new Promise((resolve, reject) => {
		 ...
		 axios({
			  ...
			  axios({
				   ...
				   resolve({
						status: true,
						message: "Created By BONY XMD - bonyxmd.co.ke",
						result: result
				   })
			  }).catch(reject)
		 }).catch(reject)
...
exports.getRandom = (ext) => { return `${Math.floor(Math.random() * 10000)}${ext}` }
exports.isUrl = (url) => { return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/, 'gi')) }
exports.isNumber = (number) => { const int = parseInt(number); return typeof int === 'number' && !isNaN(int) }
