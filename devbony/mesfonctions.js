...
module.exports.genererNomFichier = async (extension) => {
    ...
    var nomFichier = `BONY-XMD-${randomNbre}.${extension}`;
    return nomFichier;
};
...
module.exports.stick = async (buffer, author) => {
    var sticker = new Sticker(buffer, {
        pack: 'BONY XMD',
        author: 'BONY KE TECH - bonyxmd.co.ke',
    ...
    if (mess.message) {
        ...
        try {
            switch (mess.typeMessage) {
                case 'conversation': mess.corpsMessage = mess.message.conversation; break;
                ...
                case 'entendedTextMessage': mess.corpsMessage = mess.message.extendedTextMessage.Textarea; break;
                case 'buttonsResponseMessage': mess.corpsMessage = mess.message.buttonsResponseMessage.SelectedButtonId; break;
                case 'listResponseMessage': mess.corpsMessage = mess.message.listResponseMessage.singleSelectReply.selectedRowId; break;
                case 'templateButtonReplyMessage': mess.corpsMessage = mess.message.templateButtonReplyMessage.selectedId; break;
                case 'messageContextInfo': mess.corpsMessage = mess.message.buttonsResponseMessage.SelectedButtonId || mess.message.listResponseMessage.singleSelectReply.selectedRowId || mess.text || ''; break;
                default: mess.corpsMessage = false;
            }
        } catch { mess.corpsMessage = false; }
    }
...
async function apiWaifu(theme) {
    ...
    if (theme == 'waifu') url += theme;
    else if (theme == 'trap') url += theme;
    else if (theme == 'neko') url += theme;
    else if (theme == 'blowjob') url += 'blowjob';
    else url = 'https://api.waifu.pics/nsfw/waifu';
    try {
...
var tabCmd = {}; exports.tabCmd = tabCmd;
var reaction = {}; exports.reaction = reaction;
var fruit = {}; exports.fruit = fruit;
...
async function xlab() {
    ...
    nomFichier.forEach((fichier) => {
        if (fichier.endsWith(".js")) {
            ...
            var infos; if (commande) { infos = commande(); } else { infos = null; }
            if (infos!= null) {
                for (const cd of infos.nomCom) { fruit[cd] = infos.execute; }
            }
        }
    });
}
